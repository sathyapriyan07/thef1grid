...
export default function EntityDetail({ kind }: { kind: 'driver' | 'team' }) {
  const { id = '' } = useParams()
  const [teamYear, setTeamYear] = useState('ALL')
  const [teamDriver, setTeamDriver] = useState('ALL')
  ...
  const teamYearDriverStats = useMemo(() => {
    const years = new Map<string, Map<string, { driverId: string; races: number; points: number; best: number | null }>>()
    linkedResults.forEach((result) => {
      const race = races.data?.find((item) => item.id === result.race_id)
      const year = String(race?.season_id ? seasonYears.get(race.season_id) ?? 'Unknown' : 'Unknown')
      const driversForYear = years.get(year) ?? new Map()
      const current = driversForYear.get(result.driver_id) ?? { driverId: result.driver_id, races: 0, points: 0, best: null }
      current.races += 1
      current.points += Number(result.points ?? 0)
      current.best = result.position != null && (current.best == null || result.position < current.best) ? result.position : current.best
      driversForYear.set(result.driver_id, current)
      years.set(year, driversForYear)
    })
    return years
  }, [linkedResults, races.data, seasonYears])
  const teamYears = [...teamYearDriverStats.keys()].sort((a, b) => b.localeCompare(a, undefined, { numeric: true }))
  const selectedTeamDrivers = teamYear === 'ALL'
    ? [...new Map([...teamYearDriverStats.values()].flatMap((driversForYear) => [...driversForYear.values()]).map((summary) => [summary.driverId, summary])).values()]
    : [...(teamYearDriverStats.get(teamYear)?.values() ?? [])]
  const teamDriverIds = linkedDrivers.map((linkedDriver) => linkedDriver.id)
  const selectedTeamResults = linkedResults.filter((result) => {
    if (teamDriver !== 'ALL' && result.driver_id !== teamDriver) return false
    if (teamYear === 'ALL') return true
    const race = races.data?.find((item) => item.id === result.race_id)
    const year = race?.season_id ? seasonYears.get(race.season_id) : race?.date?.slice(0, 4)
    return String(year ?? '') === teamYear
  })
  ...
  return (
    <section className={`detail-page entity-${kind}-page`}>
      ...
      <section className="race-detail-section"><div className="section-heading"><div><p className="eyebrow">RESULTS RELATIONSHIP</p><h2>{kind === 'driver' ? 'Race results' : 'Drivers raced with'}</h2></div><span className="detail-count">{kind === 'driver' ? linkedResults.length : selectedTeamDrivers.length} {kind === 'driver' ? 'RACES' : 'DRIVERS'}</span></div>{!linkedResults.length ? <p className="detail-empty">No imported race results link to this record yet.</p> : kind === 'driver' ? <div className="standings-table"><div className="table-head"><span>RACE</span><span>TEAM</span><span>POS</span><span>PTS</span></div>{linkedResults.map((result) => <div className="table-row" key={result.id}><span>{raceLabel(result.race_id)}</span><span className="team-cell">{teamNames.get(result.team_id ?? '') ?? 'Unknown team'}</span><span>{result.position ?? '-'}</span><strong>{result.points ?? 0}</strong></div>)}</div> : <><div className="team-year-tabs" role="tablist" aria-label="Team driver seasons"><button type="button" className={teamYear === 'ALL' ? 'active' : ''} onClick={() => setTeamYear('ALL')}>ALL</button>{teamYears.map((year) => <button type="button" key={year} className={teamYear === year ? 'active' : ''} onClick={() => setTeamYear(year)}>{year}</button>)}</div><div className="driver-roster-card-grid">{selectedTeamDrivers.map((summary) => <Link className="driver-roster-card" key={summary.driverId} to={`/drivers/${summary.driverId}`}><div className="driver-roster-card-copy"><p className="eyebrow">DRIVER</p><h3>{driverNames.get(summary.driverId) ?? 'Unknown driver'}</h3><p className="driver-roster-period">{teamYear === 'ALL' ? 'Career with team' : teamYear}</p><p className="team-driver-card-stats">{summary.races} RACES / {summary.points} PTS / BEST {summary.best ?? '-'}</p></div><div className="driver-roster-card-image">{podiumImages.get(summary.driverId) ? <img src={podiumImages.get(summary.driverId)} alt={`${driverNames.get(summary.driverId) ?? 'Driver'} podium`} /> : <span>NO IMAGE</span>}</div></Link>)}</div></>}</section>
      {kind === 'team' && <section className="race-detail-section team-driver-results-section"><div className="section-heading"><div><p className="eyebrow">TEAM DRIVER RESULTS</p><h2>Race Results by Driver</h2></div><span className="detail-count">{selectedTeamResults.length} RESULTS</span></div><div className="team-driver-tabs" role="tablist" aria-label="Team drivers"><button type="button" className={teamDriver === 'ALL' ? 'active' : ''} onClick={() => setTeamDriver('ALL')}>ALL DRIVERS</button>{teamDriverIds.map((driverId) => <button type="button" key={driverId} className={teamDriver === driverId ? 'active' : ''} onClick={() => setTeamDriver(driverId)}>{driverNames.get(driverId) ?? 'Unknown driver'}</button>)}</div><div className="team-result-card-grid">{selectedTeamResults.map((result) => { const race = races.data?.find((item) => item.id === result.race_id); const status = statusNames.get(result.status_id ?? '') ?? (result.position ? 'Finished' : 'Unknown'); return <article className="team-result-card" key={result.id}><div className="team-result-card-head"><div><p className="eyebrow">{race?.date ?? 'DATE UNKNOWN'}</p><h3>{race?.name ?? 'Unknown race'}</h3><span>{race?.circuit_id ? circuitNames.get(race.circuit_id) ?? '-' : race?.circuit ?? '-'}</span></div><strong>{result.position ?? '-'}</strong></div><div className="team-result-card-driver"><span>{driverNames.get(result.driver_id) ?? 'Unknown driver'}</span><small>{status}</small></div><div className="team-result-card-stats"><div><span>GRID</span><strong>{result.grid ?? '-'}</strong></div><div><span>FINISH</span><strong>{result.position ?? '-'}</strong></div><div><span>PTS</span><strong>{result.points ?? 0}</strong></div><div><span>LAPS</span><strong>{result.laps ?? '-'}</strong></div></div></article>})}</div></section>}
    </section>
  )
}
