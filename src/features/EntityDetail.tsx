...
export default function EntityDetail({ kind }: { kind: 'driver' | 'team' }) {
  ...
  const linkedTeams = kind === 'driver' ? [...new Set(linkedResults.map((result) => result.team_id).filter((teamId): teamId is string => Boolean(teamId)))].map((teamId) => teams.data?.find((row) => row.id === teamId)).filter((row): row is NonNullable<typeof teams.data>[number] => Boolean(row)) : []
  ...
  const raceLabel = (raceId: string) => { const race = races.data?.find((race) => race.id === raceId); const year = race?.season_id ? seasonYears.get(race.season_id) : undefined; return `${year ? `${year} ` : ''}${race?.name ?? 'Unknown race'}` }
  ...
  return <section className="detail-page">
    ...
    <p className="eyebrow">IMPORTED DATABASE RECORD / {kind.toUpperCase()}</p><h1>{title}</h1>
    ...
    {kind === 'driver' && <section className="race-detail-section"><div className="section-heading"><div><p className="eyebrow">CAREER LINKS</p><h2>Teams</h2></div><span className="detail-count">{linkedTeams.length} TEAMS</span></div>{linkedTeams.length ? <div className="driver-roster">{linkedTeams.map((linkedTeam) => <Link className="roster-driver" key={linkedTeam.id} to={`/teams/${linkedTeam.id}`}><span className="driver-roster-mark" />{linkedTeam.name}<span>→</span></Link>)}</div> : <p className="detail-empty">No imported team results are linked to this driver yet.</p>}</section>}
