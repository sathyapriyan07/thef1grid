...
export default function EntityDetail({ kind }: { kind: 'driver' | 'team' }) {
  ...
  return (
    <section className={`detail-page entity-${kind}-page`}>
      ...
      {kind === 'driver' && <section className="race-detail-section"><div className="section-heading"><div><p className="eyebrow">RACE PARTNERS</p><h2>Teammates</h2></div><span className="detail-count">{teammateSummary.length} RELATIONSHIPS</span></div>{teammateSummary.length ? <div className="standings-table"><div className="table-head"><span>DRIVER</span><span>TEAM</span><span>PERIOD</span></div>{teammateSummary.map((summary) => { const years = [...summary.years].sort((a, b) => a - b); const period = years.length ? `${years[0]}${years.length > 1 ? `-${years[years.length - 1]}` : ''}` : 'Period unknown'; return <div className="table-row teammate-row" key={`${summary.driverId}-${summary.teamId}`}><span className="driver-cell">{teamLogos.get(summary.teamId) ? <img className="table-team-logo" src={teamLogos.get(summary.teamId)} alt="" /> : null}{driverNames.get(summary.driverId) ?? 'Unknown driver'}</span><span className="team-cell">{teamNames.get(summary.teamId) ?? 'Unknown team'}</span><strong>{period}</strong></div> })}</div> : <p className="detail-empty">No imported teammate relationships are linked to this driver yet.</p>}</section>}
